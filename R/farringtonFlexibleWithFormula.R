...
#' @export
farringtonFlexibleWithFormula <- function(sts, formula, covariates, control = list(
  ...
  if (!all(names(control) %in% names(defaultControl))) {
    inds <- which(!(names(control) %in% names(defaultControl)))
    unkonwn_parameters <- paste(names(control)[inds], collapse = ", ")
    stop("unknown control parameter(s): ", unkonwn_parameters)
  }
  ...
  formula_terms <- as.character(formula)
  if (length(formula_terms) < 3) stop("need LHS in formula")

  target_name <- formula_terms[2]
  if (!(target_name %in% colnames(observed))) stop("LHS must be in observed")

  j <- which(colnames(observed) == target_name)
  covariates_formula <- formula_terms[3]
  ...
  for (k in control$range) {
    ...
    finalModel <- algo.farrington.glm(dataGLM,timeTrend=control$trend,populationOffset=control$populationOffset,
                                      ...
                                      covariates_now = covariates[k, , drop=FALSE],
...
formulaGLM <- function(populationOffset=FALSE,timeBool=TRUE,factorsBool=FALSE,
  ...
  formulaString <- paste(formulaString, "+", covariates_formula)
...
algo.farrington.fitGLM.flexible <- function(dataGLM,
  ...
  theModel <- formulaGLM(populationOffset,timeBool=timeTrend,factorsBool, covariates_formula)
